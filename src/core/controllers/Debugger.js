...
pd.Debugger = cc.Class.extend({/**@lends pd.Debugger#*/
    ...
    /**
     * Adiciona um atalho.
     * @param targetSceneName {String|null} - o nome da cena para o qual deseja-se navegar. Ex: "GameScene" para navegar para pd.delegate.activeNamespace.GameScene.
    ...
    loadShortcut: function(id) {
        ...
        if(shortcut.targetSceneName) {
            const targetSceneName = new pd.delegate.activeNamespace[shortcut.targetSceneName]();
            var transition = FadeTransition(0.5, targetSceneName);
