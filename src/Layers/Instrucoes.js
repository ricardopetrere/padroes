...
pd.TutorialScene = cc.LayerColor.extend({
	ctor:function(caller, p){
		...
		if(!p)
			this.layerPai.pausarCena();
		
		this.layerPai.getParent().addChild(this, 99999);
		...
		this.btn_dir.setVisible(activeGameSpace.tutoriais.length>1);
		...
		this.texto_objetivo = pd.cText(512, 660, activeGameSpace.tutoriais.txt_objetivo, "Calibri", 25);
		this.addChild(this.texto_objetivo, 99);
		this.texto_objetivo.setOpacity(0);
		
		var tamanho = activeGameSpace.tutoriais.length;
		var xInicial = -(tamanho/2 - 1) * 20;
		this.vBolinhas = [];
		for(var i = 0; i < tamanho; i++){
			var bolinha = pd.cObject("stage_instrucoes", 512 + xInicial + i * 20, 130 + activeGameSpace.tutoriais.txtOffSetY, this, 50);
			bolinha.setScale(0);
			this.vBolinhas.push(bolinha);
		}
		//inicia Tweens
		for(var i = 0; i < activeGameSpace.tutoriais.length - 1; i++){
			activeGameSpace.tutoriais[i].setCascadeOpacityEnabled(true);
			activeGameSpace.tutoriais[i].setOpacity(0);
		}
	...
	startInitialTweens:function(){
		...
		var seq = new cc.Sequence(fadeScene, new cc.TintTo(0.3, 101, 167, 228), new cc.Spawn(move_background_01, move_background_02, move_background_03)
	...
	changeLayer:function(){
		...
		this.activeLayer = activeGameSpace.tutoriais[this.tutorialIndex];
		if(this.activeLayer) {
	...
	proxFunc:function(caller, arg){
		if(caller.isVisible() && this.isControlEnabled && !arg){
			...
			var size = activeGameSpace.tutoriais.length -1;
...
pd.tutorialLayer = cc.Layer.extend({
	...
	criarTextoInferior:function(txt){
		...
		if(!activeGameSpace.tutoriais.txtOffSetY)
			activeGameSpace.tutoriais.txtOffSetY = 0;
		txt = pd.cText(512, 120 + activeGameSpace.tutoriais.txtOffSetY, txt, "Calibri", 25);
		this.addChild(txt);
	},
	update:function(){
